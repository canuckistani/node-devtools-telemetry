...
class DevtoolsTelemetry {
  ...
  getProbes(version, callback) {
    var devtools_measures = [];
    this.telemetryInstance.measures(version, (measures) => {
      var probe_names = Object.keys(measures);
      var devtools_keys = probe_names.filter((name) => {
        return (name.indexOf('DEVTOOLS_') !== -1);
      });
      var out = {};
      devtools_keys.forEach((key) => {
        out[key] = measures[key];
      });
      this.DevtoolsMeasures = out;
      callback(out);
    });
  }

  // generate a model of the tools measures
  generateModel(version, callback) {
    this.telemetryInstance.measures(version, (measures) => {
      var probe_names = Object.keys(measures);
      ...
      var _measures = {};
      _.each(devtools_keys, (key) => {
        _measures[key] = measures[key];

        var parts = key.split('_', 2); var tool = parts[1].toLoweCase();
        if (!this.map.devtools[tool]) {
          this.map.devtools[tool] = [];
        }
        measure.name = name;
        this.map.devtools[tool].push(measure);
      });
      callback(this.map);
    });
  };

  getMeasuresByChannel(measureName, channel, versions, callback) {
    var length = versions.length, results = [], count = 0;
    _.each(versions, (item) => {
      var target = channel+'/'+item;

      this.telemetryInstance.loadEvolutionOverBuilds(target,
        measureName,
        (histogram) => {
          count++;
  ...
  getUsageGraph(version, name, callback) {
    this.telemetryInstance.loadEvolutionOverBuilds(version, name, (evolution) => {
      var results = {
        yes: 0,
        no: 0,
        total: 0
      ...
      var _i = 0;
      evolution.each((date, histogram, index) => {
        _i++;
  ...
  getBucketsForTool(measure, version, ranges, callback) {
    var results = _.map(_.range(ranges.length), () => { return 0; });
    var subs = 0;
    this.telemetryInstance.loadEvolutionOverBuilds(version, measure, (evolution) => {
      var result = {};
  ...
  getDailyToolUsage(windows, toolName, callback) {
    var collected = {};
  ...
  _getFunctionsFromWindows(windows, toolName) {
    var that = this;
    var functions = _.map(windows, function(win) {
      var outer = _.map(win, function(version, channel) {
  ...
  getWeeklyToolUsage(windows, toolName, callback) {
    var collected = {};
    // in this case 'window' is an array with telemetry-friendly version strings eg aurora/29
    // loop through the windows
  ...
  getWeeklyChannelUsage(windows, toolName, callback) {
    var functions = this._getFunctionsFromWindows(windows, toolName);
  ...
  generateBuildWindows(startNightly, endNightly) {
